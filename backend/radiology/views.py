...
def patient_results_list(request):
    ...
    # Show patients that have radiology orders
    patients = Patient.objects.filter(orders__order_category='radiology', is_deleted=False).distinct().order_by('-created_at')
...
def pending_orders(request):
    ...
    orders = Order.objects.filter(order_category='radiology', status='pending').order_by('order_date')
...
def fulfill_order(request, pk):
    ...
    order = get_object_or_404(Order, pk=pk, order_category='radiology')
