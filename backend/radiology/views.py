...
def patient_results_detail(request, pk):
    ...
    results = patient.test_results.select_related('test').filter(order__order_type='radiology').order_by('-performed_date', 'test__test_name')
...
def pending_orders(request):
    ...
    query = Q(order_type='radiology', test__isnull=False)
    ...
    return render(request, 'radiology/pending_orders.html', context)
...
def fulfill_order(request, pk):
    ...
    # Fetch all pending radiology orders for this patient
    ...
    if not pending_orders:
        messages.info(request, f"No pending radiology orders found for {patient}.")
    ...
    return render(request, 'radiology/fulfillment.html', context)

def group_list(request):
    groups = DiagnosticGroup.objects.filter(is_deleted=False, department__name='radiology').order_by('name')
    paginator = Paginator(groups, 10)
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
    return render(request, 'radiology/group_list.html', {'groups': page_obj})
...
def group_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('radiology:group_list')
        else:
...
def category_list(request):
    categories = DiagnosticCategory.objects.filter(is_deleted=False, group__department__name='radiology').select_related('group').order_by('group__name', 'name')
...
def category_edit(request, pk):
    ...
    return render(request, 'radiology/type_form.html', {'form': form, 'title': f'Edit Type: {category.name}'})
...
def test_list(request):
    ...
    groups = DiagnosticGroup.objects.filter(is_deleted=False, department__name='radiology').order_by('name')
    categories = DiagnosticCategory.objects.filter(is_deleted=False, group__department__name='radiology').order_by('name')
