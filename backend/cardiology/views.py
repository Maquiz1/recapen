...
def category_list(request):
    categories = DiagnosticGroup.objects.filter(is_deleted=False, department__name='cardiology').order_by('name')
...
def type_list(request):
    types = DiagnosticCategory.objects.filter(is_deleted=False, group__department__name='cardiology').select_related('group').order_by('group__name', 'name')

    paginator = Paginator(types, 10)
    page_number = request.GET.get('page')
    page_obj = paginator.get_page(page_number)
    return render(request, 'cardiology/type_list.html', {'types': page_obj})
...
def type_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('cardiology:type_list')
        else:
            messages.error(request, 'Please correct the errors below.')
    else:
        form = TypeForm()
...
def type_edit(request, pk):
    ...
    return render(request, 'cardiology/type_form.html', {'form': form, 'title': f'Edit Type: {type_obj.name}'})
...
def test_list(request):
    ...
    categories = DiagnosticGroup.objects.filter(is_deleted=False, department__name='cardiology').order_by('name')
    types = DiagnosticCategory.objects.filter(is_deleted=False, group__department__name='cardiology').order_by('name')
